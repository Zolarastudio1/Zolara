...
export function EditGiftCardDialog({ open, onOpenChange, giftCard, onUpdated }: Props) {
  ...
  useEffect(() => {
    const loadServices = async () => {
      const { data, error } = await supabase.from("services").select("id, name, category");
      if (!error && data) {
        setServices(data);
  ...
  useEffect(() => {
    if (giftCard) {
      ...
      setExpiresAt(giftCard.expires_at ? giftCard.expires_at.slice(0, 10) : "");
      setNote("");
  ...
  const handleSubmit = async () => {
    ...
    try {
      const { data, error } = await (supabase as any).rpc("rpc_update_gift_card", {
        p_id: giftCard.id,
        p_card_value: cardValue,
        p_tier: tier || null,
        p_allowed_service_ids: selectedServiceIds.length > 0 ? selectedServiceIds : null,
        p_allowed_service_categories: selectedCategories.length > 0 ? selectedCategories : null,
      ...
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-4 py-4">
          <div className="p-3 bg-muted rounded">
            ...
            <p className="text-sm text-muted-foreground">Status: {giftCard.status}</p>
          </div>
          ...
          <div>
            <Label className="mb-2 block">Allowed Services (leave empty for all)</Label>
          ...
          <div>
            <Label className="mb-2 block">Allowed Categories (leave empty for all)</Label>
            <div className="flex flex-wrap gap-2">
              {categories.map((cat) => (
                <label key={cat} className="flex items-center gap-2 text-sm cursor-pointer border rounded px-2 py-1 hover:bg-muted">
                  <Checkbox
